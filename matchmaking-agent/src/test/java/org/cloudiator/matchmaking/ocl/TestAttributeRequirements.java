...
public class TestAttributeRequirements {
  ...
  public static final Set<AttributeRequirementInOCL> TEST_REQUIREMENTS = new HashSet<AttributeRequirementInOCL>() {{
    add(inRequirement());
  }};

  private static AttributeRequirementInOCL inRequirement() {
    //add("nodes->forAll(n | Set{'test','test2','test3'}->includes(n.location.geoLocation.country))");
    return (AttributeRequirementInOCL) AttributeRequirementBuilder.newBuilder()
