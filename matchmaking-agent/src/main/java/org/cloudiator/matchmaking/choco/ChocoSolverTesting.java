...
public class ChocoSolverTesting {
  ...
  public ChocoSolverTesting(Set<NodeCandidate> possibleNodes) {
  ...
  private Solution solve(int numberOfNodes, TimeLimit timeLimit, Solution existingSolution) {
    ...
    for (NodeCandidate nc : nodesToConsider) {
      ...
      countryPrices.compute(nc.getLocation().getGeoLocation().getCountry(),
          (k, v) -> v == null ? nc.getPrice() : v + nc.getPrice());
      countryCounts.compute(nc.getLocation().getGeoLocation().getCountry(),
          (k, v) -> v == null ? 1 : v + 1);
    ...
    for (NodeCandidate nc : nodesToConsider) {
      ...
      //location stuff
      int country = countryMapper.applyAsInt(nc.getLocation().getGeoLocation().getCountry());
...
/*      //location -> cloud map creation
    ...
    for (NodeCandidate nc : existingSolution.getList()) {
      ...
      IntVar country = model.intVar("Country Existing Node " + e,
          countryMapper.applyAsInt(nc.getLocation().getGeoLocation().getCountry()));
